...
class SAGate(nn.Module):
    def __init__(self, in_planes, out_planes, reduction=16, bn_momentum=0.0003):
        ...
        self.fsp_mono = FSP(in_planes, out_planes, reduction)
        self.fsp_depth = FSP(in_planes, out_planes, reduction)

        self.gate_mono = nn.Conv2d(in_planes*2, 1, kernel_size=1, bias=True)
    ...
    def forward(self, x):
        ...
        rec_mono = self.fsp_mono(depth, mono)
        rec_depth = self.fsp_depth(mono, depth)

        cat_fea = torch.cat([rec_mono, rec_depth], dim=1)

        attention_vector_l = self.gate_mono(cat_fea)
        ...
        merge_feature = mono*attention_vector_l + depth*attention_vector_r

        mono_out = (mono + merge_feature) / 2
        depth_out = (depth + merge_feature) / 2

        mono_out = self.relu1(mono_out)
        depth_out = self.relu2(depth_out)

        return [mono_out, depth_out], merge_feature