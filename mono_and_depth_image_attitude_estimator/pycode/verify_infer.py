...
class VerifyInfer:
    ...
    def data_load(self):
        ...
        with open(csv_path) as csvfile:
            ...
            for row in reader:
                ...
                roll_val_list.append(row[6])
                pitch_val_list.append(row[7])
    ...
    def spin(self):
        ...
        for row in self.data_list:
            tmp_roll_diff = abs(float(row[2]) - float(row[4]))
            tmp_pitch_diff = abs(float(row[3]) - float(row[5]))
            ...
            acc_roll_variance += float(row[6])
            acc_pitch_variance += float(row[7])
